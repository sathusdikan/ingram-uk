...
export function formatRateForShopify(
  ...
  if (rate.distributions.length > 1) {
    const shipText = countryCode === "GB" ? "locations" : "locations";
    descParts.push(`Ships from ${rate.distributions.length} ${shipText}`);
  }

  const defaultDesc = countryCode === "GB" ? "Ingram Micro delivery" : "Ingram Micro freight";
  ...
  return {
    ...
    currency,
    description: descParts.join(" • ") || defaultDesc,
  };
}